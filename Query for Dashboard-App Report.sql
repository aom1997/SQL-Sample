...
FROM [temp_Zeming].[dbo].[tin_consultant]
WHERE 
	Provider_Consultant in(@ProviderConsultantParam)
	and multi_consultant_tin=0
	AND dataset = (SELECT TOP(1) dataset FROM [temp_Zeming].[dbo].[tin_consultant] ORDER BY dataset DESC)
create index TIN_NPI on #tTinConsultantMapping(TIN,NPI)

--Consultants from Zemings database (NPI BASED FOR multi consultant tins)
